 ...
 */
class Zend_Tag_Cloud_Decorator_WishTag extends Zend_Tag_Cloud_Decorator_Tag
{
    /**
    ...
    public function render(Zend_Tag_ItemList $tags)
    {
        ...
        foreach ($tags as $tag) {
            ...
            // add ID to Tag
            $idParam = $tag->getParam('ID');
            if(isset($idParam)){
                $attribute .= " ";
                $attribute .= sprintf('ID="%s"', htmlspecialchars($tag->getParam('ID'), ENT_COMPAT, $enc));
            }
